...
SELECT
  ...
  -- Acomptes
  COALESCE((
    SELECT SUM(a.montant_xaf) FROM acomptes a
    WHERE a.commande_id = cmd.id AND a.statut = 'VALIDE'
...
WITH conso_30j AS (
  ...
  WHERE bsa.date_sortie >= CURRENT_DATE - 30
...
avec_delai AS (
  SELECT
    ...
    COALESCE(fp.delai_livraison_jours, f.delai_livraison_jours, 7) AS delai_fourn_jours,
